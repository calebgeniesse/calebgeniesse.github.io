...
- **[`ReciprocalIsomap`](https://github.com/calebgeniesse/reciprocal_isomap).**&nbsp;&nbsp;A reciprocal variant of Isomap for robust non-linear dimensionality reduction in Python. `ReciprocalIsomap` was inspired by scikit-learn's implementation of Isomap, but the reciprocal variant enforces shared connectivity in the underlying *k*-nearest neighbors graph (i.e., two points are only considered neighbors if each is a neighbor of the other).
...
- **[`PartialCluster`](https://github.com/calebgeniesse/pcluster).**&nbsp;&nbsp;A partial clustering wrapper for Mapper. `PartialCluster` wraps around a user-specified clustering algorithm to enable globally-aware partial clustering. By pre-clustering an entire dataset, *local* subsets of the data can be partially clustered based on their *global* cluster assignments.
